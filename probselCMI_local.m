%% This script implements the UPenn & iEEG.org functions in order to run sCMI (or other kind of CMI?) on the data for the probilisitic reinforcement learning task as in Frank, Ramayya, and Zaghloul papers. 

%% load data.
psPath = '/home/elliot/data/probsel2/'
dirlist = dir('*.mat')
...
for sb = 1:length(dirList.name)
	
	% getting subject events. 
	%% [did this] [events evDeets] = get_sub_events(task,subjects{sb}); 

	% [20160301] getting event types. 
	%% [did this] evType = getStructField(events,'evType');
	%% [did this] stimIdx = cellfun(@isequal,evType,repmat({'stim'},size(evType)));
    %% [did this] fbIdx = cellfun(@isequal,evType,repmat({'fb'},size(evType)));

	% [20160301] getting specific event times. 
	%% [did this] stimTimes = [events(stimIdx).mstime];
	%% [did this] fbTimes = [events(fbIdx).mstime];  	
